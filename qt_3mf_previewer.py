...
G90; 切换回绝对坐标模式"""
    ...
    def compare_color_change_codes(self):
        ...
        for line in lines:
            line = line.strip()
            if line.startswith('M620.1'):
                # 提取完整的M620.1行
                m620_1_lines.append(line)
        
        # 分析差异
        if not m620_1_lines:
            QMessageBox.information(self, "分析结果", "未找到换色代码(M620.1)！")
            return
        
        # 检查是否所有行都相同
        all_same = all(line == m620_1_lines[0] for line in m620_1_lines)
        ...
        if all_same:
            ...
            result += f"代码内容: {m620_1_lines[0]}\n"
        else:
            result += "状态: 存在换色\n"
            result += "提示: 会使用不同颜色打印\n"
            result += "\n各换色代码内容:\n"
            for i, line in enumerate(m620_1_lines):
                result += f"{i+1}. {line}\n"
        ...
        self.color_change_analysis['codes'] = m620_1_lines
